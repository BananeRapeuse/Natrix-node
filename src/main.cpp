...
#if defined(NDEBUG)
# error "Natrix cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe373e2c9bd19b58f4cdf953bc75da235e85958400854d883559291d16a4250ac");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Natrix: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Natrix Signed Message:\n";
...
{
    // Natrix: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 57 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Natrix: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Natrix: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // Natrix: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Natrix: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Natrix: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1ceed8ec81dad6ad11062376a36478e2fafac22e3728f1c8b2351b6b3c0a7324");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 20/07/2026 - Every Miku is canon they said";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 57 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04108cac56ace78998db08a138891521ea2f61c3ed39dbf2a41b5c971baa18fc933b7d139fd676e5695063dc41a9054b93e328769066db4c1a39528b66e4fa0e9f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1784577379;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086757938;

        if (fTestNet)
        {
            block.nTime    = 1784577335;
            block.nNonce   = 385381737;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf3ba67b1f33c825d1a927255feda21d9aee1694c5ab962a191889b2fd1a1f2f6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc0, 0xb2, 0xdc }; // Natrix: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NatrixMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NatrixMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NatrixMiner(CWallet *pwallet)
{
    printf("NatrixMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("natrix-miner");
    ...
    try { loop {
        ...
        printf("Running NatrixMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NatrixMiner, pwallet));
}
